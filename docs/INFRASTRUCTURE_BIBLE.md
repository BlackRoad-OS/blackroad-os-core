...
docker buildx build   --platform linux/amd64,linux/arm64   -t ghcr.io/blackroad-os/api:latest   --push .
```
      ...
      - name: Deploy to Pi
        ...
        with:
          ...
          script: |
            ...
            docker rm api || true
            docker run -d --name api               -p 8000:8000               --restart unless-stopped               ghcr.io/${{ github.repository }}:latest
```
...
# Pull and run vLLM
docker run --runtime nvidia --gpus all   -v ~/.cache/huggingface:/root/.cache/huggingface   -p 8080:8000   --restart unless-stopped   vllm/vllm-openai:latest   --model Qwen/Qwen2.5-7B-Instruct   --max-model-len 4096
```
